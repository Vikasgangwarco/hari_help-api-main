...
router.get("/patient", async (req, res) => {
  try {
    ...
    // let ret={};
    ret.message = "Patient found successfully";
    ret.status = "success";
    ret.data = [patient];
...
router.post("/patient_delete", async (req, res, next) => {
  let ret = {
    message:
      "Sorry you are not allowed to perform this task, permission denied.",
    status: "failed",
  ...
  try {
    ...
    console.log(req.user._id);
    await Patients.deleteOne({ _id: req.user._id });
    ret.status = "success";
    ret.message = "Patient deleted successfully";
  } catch (err) {
...
router.post("/patient_update", async (req, res, next) => {
  let ret = {
    message:
      "Sorry you are not allowed to perform this task, permission denied.",
    status: "failed",
...
// // Route to create an appointment for a patient
// router.post("/patient_create_appointment", async (req, res, next) => {
...
//      // Check if doctor OR patient exist
...
//   let ret = { status: "failed", message: "" };
//   const appointmentData = req.body;
//   const patientId = req.user._id;  // Assume patient ID comes from authentication middleware

//   try {
//     // Check if the authenticated user is a patient
//     if (req.user.role !== "user") {
//       ret.message = "Access forbidden: Only patients can create appointments.";
//       // return res.status(403).json(ret);
//       return res.status(200).json(ret);

//     }

//     // Validate that appointment data has required fields
//     // if (!appointmentData.doctors || !appointmentData.appointment_date || !appointmentData.appointment_time) {
//       if (!appointmentData.doctors || !appointmentData.appointment_date ) {

//       ret.message = "Required appointment fields missing.";
...
//     // You can also add additional checks like if the doctor is available at the requested time, 
//     // if the time slot is not taken, etc.

//     // Create a new appointment
//     const newAppointment = new Appointments({
//       patients: [patientId],  // Assuming each appointment is made by one patient
//       doctors: [appointmentData.doctors],  // Assuming each appointment is with one doctor
//       appointment_date: appointmentData.appointment_date,
//       // appointment_time: appointmentData.appointment_time,
//       reason_for_visit: appointmentData.reason_for_visit,
//       notes: appointmentData.notes,
...
//   } catch (err) {
//     console.log("Some error occurred:", err);
//     ret.message = "An error occurred while creating the appointment.";
//     // return res.status(500).json(ret);
//     return res.status(200).json(ret);
//   }
//   return res.status(200).json(ret);
...
router.post("/patient_create_appointment", async (req, res, next) => {
  ...
  try {
    ...
    if (!appointmentData.doctors || !appointmentData.appointment_request_date) {
      ret.message = "Required appointment fields missing (doctor or appointment date).";
...
// if(appointmentData.payment_transaction_id==!null){
//   appointmentData.payment_status="paid";
//   ret.message = "Payment transaction is sucessfull";
//   ret.data = payment_status;
//   res.status(200).json(ret);  

// }else{
//   appointmentData.payment_status="unpaid";
//   ret.message = "Payment transaction is not sucessfull";
//   ret.data = payment_status;
//   res.status(200).json(ret);  
// }

// if(appointmentData.payment_transaction_id==""||appointmentData.payment_transaction_id==null){
//   appointmentData.payment_status="unpaid";
//   ret.message = "Payment transaction is not sucessfull";
//   ret.data = payment_status;
//   res.status(200).json(ret);  
...
// }
    ...
    // if (appointmentData.payment_transaction_id==null || appointmentData.payment_transaction_id == "") {
    //   appointmentData.payment_status = "unpaid";
    //   ret.message = "Payment transaction is successful";
    // } else {
    //   appointmentData.payment_status = "paid";
    //   ret.message = "Payment transaction is successful";
    // }
  ...
  // Set payment status based on the presence of a payment transaction ID
  appointmentData.payment_status = appointmentData.payment_transaction_id ? "paid" : "unpaid";
...
router.post("/patient_edit_appointment/:appointment_id", async (req, res, next) => {
  let ret = { status: "failed", message: "" };
  const appointmentData = req.body;
  const patientId = req.user._id;  // Assuming patient ID comes from authentication middleware
  ...
  try {
    // Check if the authenticated user is a patient
    if (req.user.role !== "user") {
      ret.message = "Access forbidden: Only patients can edit appointments.";
      return res.status(403).json(ret);  // Return 403 Forbidden
    }

    // Validate that required appointment fields are provided
    if (!appointmentId || !appointmentData.appointment_request_date) {
    ...
    if (!appointment.patients.includes(patientId)) {
      ret.message = "You are not authorized to edit this appointment.";
    ...
    appointment.appointment_request_date = appointmentData.appointment_request_date || appointment.appointment_request_date;
    ...
    ret.status = "success";
    ret.message = "Appointment updated successfully";
...
router.get("/patient_appointments", async (req, res, next) => {
  ...
  try {
    ...
    const appointments = await Appointments.find({ patients: patientId })
      .populate('patients')
      .populate('doctors')
      .sort({ createdAt: -1 })
      .skip(skip)
     ...
     // Calculate serial numbers
        let totalAppointments = await Appointments.countDocuments({ patients: patientId });
  ...
  res.status(200).json(ret);
          ...
          // Route to get all doctors for a specific patient
router.get("/patient_get_doctors", async (req, res, next) => {
  ...
  try {
    ...
    // Find doctors for the patient
    const doctors = await Doctors.find(filterObj)
      // .populate('patients')
      // .populate('doctors')
      .sort({ updated_at: -1 })
     ...
     // Calculate serial numbers
      ...
      let doctorserial = doctors.map((doctor, index) => {
            doctor.serialNumber = totalDoctors - (skip + index);
            return doctor;
        });

    ret.status = "success";
    ret.message = "Doctors retrieved successfully";
    ret.data = doctorserial;
  } catch (err) {
    console.log("Some error occurred:", err);
    ret.message = err.message;
    // return res.status(500).json(ret);
