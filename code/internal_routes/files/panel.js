...
router.post("/patient_bulk_upload", uploadFiles.single('file'), async (req, res, next) => {
  console.log('amit');
  let ownerId = req.user._id;
  // var url = "/home/amit/api-server/content/files/" + req.name;
  // var url = "/home/amit/api/content/files/" + req.name;
  ...
  try {
      for (var i = 0; i < data.length; i++) {
          ...
          try {
              if (!(await Users.exists({ phone: patient.contactNumber })) ){
                  t = new Users({ phone: patient.contactNumber });
                  await t.save();
              } else {
                  t = await Users.findOne({ phone: patient.contactNumber });
              }

              if (await Patients.exists({ user: t._id })) {
                  // Update the existing student's information
                  const existingPatient = await Patients.findOne({ user: t._id });
                  ...
                  await existingPatient.updateOne(updatedPatientData);

                  if (patient.appointment_id) {
                      await Appointments.addPatient(patient.appointment_id, existingPatient._id);
                  }
                  ...
                  if (patient.appointment_id) {
                      await Appointments.addPatient(patient.appointment_id, f._id);
                  }
