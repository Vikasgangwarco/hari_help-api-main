...
const {performLoginUser,performLoginDoctor,performValidationUser,performValidationDoctor} = require('../../../utility/authentication_functions');
...
router.post("/login_doctor",async function(req,res,next){
    performLoginDoctor(req,res,next);
});
...
router.post("/logout_doctor",async function(req,res,next){
    var ret= {status:"failed",message:""};
    try{
        req.logout();
        ret.status = "success";
...
router.get("/whoami_doctor", performValidationDoctor, async function(req,res,next){
    //console.log(req.user);
        var ret = {};
        if(req.user)
        {
...
router.get("/logout_doctor", async function(req,res,next){
	if(req.user)
	{
		req.logout();
    }
