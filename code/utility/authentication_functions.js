...
function performLoginDoctor(req, res, next)
{
    //this callback function is called after setup.js file function has returned, its just later in that pipeline, actually, the immediate next...
    passport.authenticate("otp-strategy-doctor", function(err/* true-false */, user, info) {
        console.log("Entered authenticate");
        if (err) {
            console.log("err authenticate"+err);
            return res.status(400).json({ status:"failed",message:err });
...
function performValidationDoctor(req, res, next)
{
    //this callback function is called after setup.js file function has returned, its just later in that pipeline, actually, the immediate next...
    passport.authenticate("jwt_doctor", { session: false },/*callback=>*/function(err, user, info) {
        console.log("Entered authenticate");
        if (err) {
            console.log("err authenticate"+err);
            return res.status(400).json({ status:"failed",message:err });
...
function performLoginPanel(req, res, next)
{
    ...
    passport.authenticate("local_strategy_admin_panel", function(err, user, info) {
        ...
        req.logIn(user, function(err) {
            ...
            return res.status(200).json({ status:"success",message: `logged in ${user._id}`,user_id:user.id,data:user});
...
module.exports={performLoginUser,performLoginDoctor,performLoginPanel,performValidationUser,performValidationDoctor,performValidationPanel};